"""Aggregate per-PV U-Net predictions into sky-plane shell candidates (PyTorch)."""
...
from hishells_pv.utils.wcs_tools import pixel_scales_arcsec
from hishells_pv.data.dataset import normalize
from hishells_pv.infer.predict import load_checkpoint
...
def _tile_predict_full_pv(
    ...
    """
    pv = normalize(np.asarray(pv, dtype=np.float32), norm_method)
    V, S = pv.shape
    ...
    def _flush(batch_buf, coords):
        if not batch_buf:
            return
        x = torch.from_numpy(np.stack(batch_buf, axis=0)).unsqueeze(1).to(device)  # (B,1,ph,pw)
        probs = torch.sigmoid(model(x))[:, 0].detach().cpu().numpy()
        for (y, xpos), pr in zip(coords, probs):
            out[y:y+ph, xpos:xpos+pw] += pr
            count[y:y+ph, xpos:xpos+pw] += 1.0

    batch_buf: list[np.ndarray] = []
    coords: list[tuple[int, int]] = []
    for y0 in ys:
        for x0 in xs:
            batch_buf.append(pv_pad[y0:y0+ph, x0:x0+pw].astype(np.float32))
            coords.append((y0, x0))
            if len(batch_buf) == batch:
                _flush(batch_buf, coords)
...
def _write_regions(peaks: List[Tuple[int,int,float]], wcs: WCS, path: Path, color="cyan"):
    ...
    The vote map lives in the image (RA/Dec) plane, but THINGS cube headers carry
    3-4 WCS axes (RA, Dec, velocity, Stokes). Reduce to the 2D celestial sub-WCS
    so ``pixel_to_world_values(x, y)`` receives the right number of coordinates.
    """
    cel = wcs.celestial if getattr(wcs, "has_celestial", False) else wcs
    lines = [
    ...
    for (y, x, s) in peaks:
        ra, dec = cel.pixel_to_world_values(x, y)[:2]
...
def aggregate(
    ...
    ``output_root`` and ``cube_path`` override the config values when provided
    (useful for ad-hoc runs against a config that omits them).
    """
    cfg = resolve_config(cfg_path, write_resolved=False)
    if output_root is not None:
        cfg["output_root"] = output_root
    if cube_path is not None:
        cfg["cube_path"] = cube_path
    root = Path(cfg["output_root"])
    pv_dir = root / "pv"
    if not pv_dir.exists():
        raise FileNotFoundError(f"PV directory not found: {pv_dir}")

    # Model + patch sizes from the trained checkpoint
    device = resolve_device(device_name)
    model_path = Path(run_dir) / "best_model.pt"
    ...
    # Sky geometry. Only the header/WCS are needed (image-plane size + astrometry);
    # avoid open_cube() here so we don't load the full cube data array into memory
    # (THINGS cubes such as NGC 3031 are multi-GB and would OOM the process).
    ...
    for fname in items:
        ...
        prob = _tile_predict_full_pv(
            model, pv, ph, pw, device, stride_v=ph//2, stride_s=pw//2, batch=16, norm_method=norm_method
        )  # (V,S)
...
def _parse_args():
    ...
    ap.add_argument("--run_dir", required=True, help="Run dir with best_model.pt")
