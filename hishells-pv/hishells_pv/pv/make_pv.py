...
from hishells_pv.utils.io import load_yaml, dumps_json
from hishells_pv.utils.config import resolve_config
from hishells_pv.utils.wcs_tools import (
...
def _save_line_pv(
    *,
    ...
    if qa_enabled:
        ...
        plt.colorbar()
        plt.title(f"PV (v×pos): {stem_base}")
        plt.tight_layout()
...
# -----------------------------------------------------------------------------
# Catalog-aware shell cuts: positive coverage without replacing blind grid
# -----------------------------------------------------------------------------
