...
from hishells_pv.utils.wcs_tools import open_cube, unit_vectors_for_pa, velocity_axis_kms
...
def main(cfg: Dict):
    ...
    qa_max_plots = label_opts.get("qa_max_plots")
    qa_max_plots = None if qa_max_plots is None else int(qa_max_plots)
    ...
    slices_by_source: Counter = Counter()
    positives_by_source: Counter = Counter()
    ...
    for pv_path in pv_files:
        ...
        if meta.get("type") not in ("grid", "catalog_shell", "background_negative"):
            continue
        source_type = str(meta.get("type", "unknown"))
        slices_by_source[source_type] += 1
        ...
        if objects:
            positive_slices += 1
            positives_by_source[source_type] += 1
        ...
        if qa_max_plots is None or written < qa_max_plots:
    ...
    summary = {
        ...
        "slices_by_source": {str(k): int(v) for k, v in slices_by_source.items()},
        "positive_slices_by_source": {str(k): int(v) for k, v in positives_by_source.items()},
        "negative_slices_by_source": {
            str(k): int(v - positives_by_source.get(k, 0)) for k, v in slices_by_source.items()
        },
