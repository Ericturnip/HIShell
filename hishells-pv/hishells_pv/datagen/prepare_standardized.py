...
from hishells_pv.catalog.shell_catalog import catalog_to_pixel_shells, load_bagetakos_table7
...
from hishells_pv.utils.io import dumps_json
from hishells_pv.utils.wcs_tools import open_cube, pixel_scales_arcsec, synthesized_beam_arcsec, unit_vectors_for_pa
...
def _emit_agg_artifacts(
    out_root: Path,
    rows_by_split: dict[str, list[dict[str, Any]]],
    galaxy_cfgs: dict[str, dict[str, Any]],
    galaxy_splits: dict[str, str],
    *,
    target_velocity_bins: int,
    target_spatial_pixels: int,
    include_splits: set[str],
    only_galaxies: set[str] | None,
) -> list[str]:
    """Write per-galaxy aggregation inputs + configs; returns galaxy ids emitted."""
    files_by_galaxy: dict[str, list[str]] = defaultdict(list)
    for rows in rows_by_split.values():
        for row in rows:
            files_by_galaxy[row["galaxy"]].append(row["filename"])

    emitted: list[str] = []
    for galaxy_id, filenames in sorted(files_by_galaxy.items()):
        split = galaxy_splits.get(galaxy_id)
        cfg = galaxy_cfgs.get(galaxy_id)
        if cfg is None or split is None:
            continue
        if only_galaxies is not None:
            if galaxy_id.casefold() not in only_galaxies:
                continue
        elif split not in include_splits:
            continue
        label = split_label(galaxy_id, split)
        agg_root = build_galaxy_agg_input(out_root, out_root / "pv", galaxy_id, filenames, label)
        write_agg_config(
            agg_root,
            galaxy_id,
            cfg,
        ...
        emitted.append(galaxy_id)
        print(f"[standardized-data] emitted agg config -> {agg_root / f'{galaxy_id}_agg_config.yaml'}")
    return emitted
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument(
        ...
        default=True,
        help="Emit per-galaxy agg/<galaxy>/ inputs + aggregate-ready configs.",
    )
    ap.add_argument(
        "--agg-galaxies",
        nargs="*",
        default=None,
        help="Splits or 'all' to emit agg inputs for (default: val test stress).",
    )
    ...
    for item in _load_galaxy_manifest(args.data_root):
        ...
        galaxy_cfgs[galaxy_id] = cfg
        galaxy_splits[galaxy_id] = split
        ...
        for spec in deployment_grid_specs(
            galaxy_name,
            moment0,
            angles_deg=[0.0, 45.0, 90.0, 135.0],
            stride_pix=args.grid_stride_pix,
            max_specs=args.grid_max_per_galaxy,
        ):
            specs.append(
                StandardCutSpec(
                    galaxy=spec.galaxy,
                    center_pix=spec.center_pix,
    ...
    if args.emit_agg_configs:
        if args.agg_galaxies and any(g.casefold() == "all" for g in args.agg_galaxies):
            include_splits = {"train", "val", "test", "stress"}
            only_galaxies = None
        elif args.agg_galaxies:
            known_splits = {"train", "val", "test", "stress"}
            requested = {g.casefold() for g in args.agg_galaxies}
            include_splits = requested & known_splits
            extra_galaxies = requested - known_splits
            only_galaxies = extra_galaxies or None
            if only_galaxies is not None:
                include_splits = {"train", "val", "test", "stress"}
        else:
            include_splits = {"val", "test", "stress"}
            only_galaxies = None
        emitted = _emit_agg_artifacts(
            out_root,
            rows_by_split,
            galaxy_cfgs,
            galaxy_splits,
            ...
            only_galaxies=only_galaxies,
        )
        print(f"[standardized-data] emitted {len(emitted)} per-galaxy agg config(s) under {out_root / 'agg'}")
