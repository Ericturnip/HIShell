...
from hishells_pv.train.tf_models_unet import unet_pv
from hishells_pv.train.tf_losses import make_loss_and_metrics
from hishells_pv.train.callbacks import SegmentedValidationCallback



class EveryNEpochs(keras.callbacks.Callback):
    """
    Print a compact training line every few epochs.
    The callback keeps long training runs readable when Keras verbose output is off.
    """
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if is_first or is_multiple or is_last:
            fields = (
                ...
                "val_loss",
                "val_pr_auc",
                "val_pixel_recall_0p05",
                "val_pixel_recall_0p075",
                "val_patch_recall_0p05",
...
def _load_training_cfg(cfg_path: str) -> dict:
    """
    Load config for training, falling back to raw YAML when the TensorFlow env
    does not include the astronomy dependencies needed by the full resolver.
    """
    cfg = Path(cfg_path)
    ...
    try:
        ...
        print(f"[train] warning: using raw training config; full resolver unavailable ({exc})")
...
def train(cfg_path: str,
    ...
    """
    # Resolve when possible; raw training YAML is enough for array/mask training.
    cfg = _load_training_cfg(cfg_path)
    ...
    if cfg.get("train", {}).get("max_steps_per_epoch") is not None:
        steps_per_epoch = min(steps_per_epoch, int(cfg["train"]["max_steps_per_epoch"]))
    if cfg.get("train", {}).get("max_validation_steps") is not None:
        validation_steps = min(validation_steps, int(cfg["train"]["max_validation_steps"]))

    model = _build_model(cfg)

    model_selection = cfg.get("metrics", {}).get("model_selection", {})
    primary_monitor = str(model_selection.get("primary_monitor", "val_pr_auc"))
    primary_mode = str(model_selection.get("mode", "max"))
    high_recall_monitor = str(model_selection.get("high_recall_monitor", "val_patch_recall_0p075"))
    ...
    cbs: list[keras.callbacks.Callback] = [
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=str(out / "high_recall_model.keras"),
            monitor=high_recall_monitor,
            mode="max",
            save_best_only=True,
            save_weights_only=False,
            verbose=0,
        ...
        keras.callbacks.ReduceLROnPlateau(
            monitor=primary_monitor, mode=primary_mode,
    ...
    segmented_cfg = cfg.get("segmented_validation", {})
    if segmented_cfg.get("enabled", bool(cfg.get("standardized_pv", {}).get("enabled"))):
        cbs.append(
            SegmentedValidationCallback(
                cfg,
                out,
                split=str(segmented_cfg.get("split", "val")),
                every=int(segmented_cfg.get("every", max(1, print_every))),
                max_cuts=segmented_cfg.get("max_cuts"),
            )
        )
