...
def _load_cfg(cfg_path: str) -> Dict:
    """Load a resolved config when one exists locally."""
...
def _tile_predict_full_pv(
    ...
    """
    Predict a full PV map by averaging overlapping model tiles.
    Returns a probability map with the same velocity and spatial axes.
    """
    V, S = pv.shape
    pad_v = max(0, ph - V)
    pad_s = max(0, pw - S)
...
def _write_regions(peaks: List[Tuple[int,int,float]], wcs: Any, path: Path, color="cyan"):
    """
...
def aggregate(
    ...
    # CARTA can read this vote map as a normal FITS image.
    ...
    # CARTA and DS9 can load this region overlay.
    if write_regions:
        try:
