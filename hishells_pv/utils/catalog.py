...
def _read_dat_any(path: str) -> pd.DataFrame:
    ...
    # Astropy handles CDS/VizieR tables better than pandas when metadata is present.
...
def _looks_like_table7_tokens(tokens: List[str]) -> bool:
    """
    Detect rows that look like Bagetakos table 7 entries.
    The first token is usually a galaxy prefix such as NGC, IC, DDO, UGC, or M.
    """
    if len(tokens) < 12:
        return False
...
def _parse_table7_tokenized(path: str, target_galaxy: str | None = None) -> pd.DataFrame:
    ...
    If some fields are missing or not convertible, keep the row and use NaN.
    """
...
def load_catalogs(cfg: Dict) -> pd.DataFrame:
    """
    Load the shell catalog named in the pipeline config.
    Bagetakos table 7 is the main source; catalog_csv remains a compatibility path.
    """
