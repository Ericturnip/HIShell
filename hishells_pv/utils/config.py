...
from hishells_pv.utils.wcs_tools import velocity_axis_kms
...
SCHEMA = {
  ...
  "properties": {
    ...
    "catalog_csv": {"type":["string","null"]},
    "catalogs": {"type":["object","null"]},
    "galaxy": {
      "type":"object",
      "properties": {
...
def _env_overrides(prefix="PV_")->Dict[str,Any]:
    """
    Convert environment overrides into nested config values.
    For example, PV_model.base_filters=48 updates model.base_filters.
    """
    out: Dict[str,Any] = {}
...
def _pixel_scales_from_header(hdr) -> Tuple[Optional[float], Optional[float]]:
    """
    Estimate angular and spectral pixel scales from the FITS header.
    The angular scale is the average of CDELT1 and CDELT2 in arcsec.
    """
    pix_arcsec = None
    try:
        cdelt1 = abs(hdr.get("CDELT1") or hdr.get("CD1_1") or 0.0) * 3600.0
...
def _apply_pv_defaults_and_conversions(cfg: Dict[str, Any]) -> None:
    """Fill missing PV defaults and convert beam-based settings to pixels."""
    ...
    sampling.setdefault("pos_samples_per_beam", 3)
    sampling.setdefault("vel_smooth", 0)
    sampling.setdefault("vel_subsample", 1)
...
def _stamp_warnings_if_missing(cfg: Dict[str, Any]) -> None:
    ...
    if cfg.get("_meta", {}).get("pix_arcsec") is None:
        print("[config] WARNING: could not infer sky pixel scale (arcsec/pix) from FITS WCS; beam-to-pixel conversions may be skipped.")
