...
This repository contains the paper-facing code for detecting HI shells in position-velocity (PV) slices from THINGS galaxy cubes.
...
This is not a dump of every experiment. Historical runs, review panels, generated arrays, TensorBoard logs, and scratch figures have been left out. What remains is the finished pipeline, the report-facing `hishells_pv` package, the shell-type analysis notebook, and a retained TensorFlow/Keras cross-check checkpoint.
...
- `hishells_pv/`: paper-facing package layout. It is organized by stage: catalog loading, PV cuts and labels, PyTorch datasets, model/loss/metrics, training, and inference/aggregation.
- `scripts/`: command-line entry points for the full training workflow and smaller one-off tasks. The report-facing drivers are `generate`, `train`, `infer`, `calibrate`, `postprocess`, and `aggregate-all`.
- `training_data/configs/`: Per-galaxy YAML configs. These point to local FITS files under `data/raw/`.
- `artifacts/models/hi_shell_pv_unet_clean_physical_baseline.keras`: retained TensorFlow/Keras cross-check checkpoint.
- `artifacts/model_metadata.json`: checksum, model settings, recommended threshold, and condensed metrics for that retained checkpoint.
...
The final report describes the PyTorch `UNetPV` detector. Reproducing that path writes a run directory such as:

```text
runs/pv_unet_real/
  history_torch.csv
  best_model.pt
  high_recall_model.pt
  final_model.pt
```

Generated `runs/` directories are local products and are not tracked in Git. The retained checkpoint currently stored in the repository is the TensorFlow/Keras cross-check model:
...
The DDO 53 physical-grid check in the metadata records full shell-level coverage and detection recall for catalog shells `[1, 2, 3]` at the same threshold. Treat this metadata as the retained TensorFlow checkpoint record, not as a replacement for a fresh PyTorch `history_torch.csv`.
...
```bash
python -m hishells_pv.infer.infer_pv \
...
## Reproduce The PyTorch Detector

The report-facing detector path is:

```bash
scripts/generate \
  --out-root training_data/standardized_5kpc_200kms_clean_physical_baseline \
  --spatial-window-kpc 5.0 \
  --velocity-window-kms 200.0 \
  --target-velocity-bins 96 \
  --target-spatial-pixels 256 \
  --grid-stride-pix 32 \
  --grid-max-per-galaxy 2000 \
  --grid-angle-step-deg 22.5 \
...
That run writes `runs/pv_unet_real/history_torch.csv`, plus `best_model.pt`, `high_recall_model.pt`, and `final_model.pt`.

## Reproduce The TensorFlow Cross-Check

The TensorFlow clean physical baseline is the workflow that produced the retained Keras checkpoint. It:
...
- `scripts/generate`: generate standardized PV cuts and masks.
- `scripts/train`: train the PyTorch `UNetPV` detector.
- `scripts/infer`: run model inference over generated PV arrays.
- `scripts/calibrate`: evaluate thresholds on a named split.
- `scripts/postprocess`: evaluate connected-component filtering and ranking.
- `scripts/aggregate-all`: aggregate probability maps back toward sky/candidate outputs.
...
- `hishells_pv/catalog/`: Bagetakos table loading. `load_bagetakos_table7` is the public loader used by labeling and tests.
- `hishells_pv/pv/`: PV cut generation, physical standardization, and label painting.
- `hishells_pv/data/dataset.py`: PyTorch `PVPatchDataset` and finite-value z-score normalization.
- `hishells_pv/models/`: PyTorch `UNetPV`, `BCETverskyLoss`, and patch/pixel metrics.
- `hishells_pv/train/`: PyTorch training entry point, plus the TensorFlow training path shipped alongside it.
- `hishells_pv/infer/`: inference, threshold calibration, component extraction, post-processing, and sky aggregation.
...
scripts/generate
scripts/train
hishells_pv/train/torch.py
```

Detector-metric figures are read from a run's `history_torch.csv`. Vote maps come from the `aggregate_*` outputs written by aggregation. Catalog figures come from the shell-type notebook.
...
Run `pytest` to check catalog parsing, the PyTorch U-Net forward pass, the BCE-Tversky loss, dataset loading/normalization, and a synthetic end-to-end training step without proprietary FITS cubes.
