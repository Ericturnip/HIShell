"""
Generate the paper-style scientific figures used in the project report.
Each figure is produced from checked-in result summaries or generated PV
manifests, then exported as PNG and PDF.
"""
...
def setup() -> None:
    """Set the shared Matplotlib style for the report figures."""
...
def save(fig: plt.Figure, stem: str) -> tuple[Path, Path]:
    """Save one figure in both formats expected by the report workflow."""
    png = OUT / f"{stem}.png"
    pdf = OUT / f"{stem}.pdf"
...
def panel_label(ax, label: str) -> None:
    """Add a small panel letter to multi-panel figures."""
    ax.text(
        0.03,
        0.95,
        label,
...
def robust_image(pv: np.ndarray) -> np.ndarray:
    """Scale PV intensities for display without changing the underlying data."""
...
def figure_pv_examples() -> tuple[Path, Path]:
    """Create Figure 1: PV examples with catalog-derived shell masks."""
...
def figure_dataset_and_types() -> tuple[Path, Path]:
    """Create Figure 2: split composition and shell-type label counts."""
    manifests = []
...
def figure_training_history() -> tuple[Path, Path]:
    """Create Figure 3: loss, patch precision, and patch recall over epochs."""
...
def figure_postprocessing_metrics() -> tuple[Path, Path]:
    """Create Figure 4: patch metrics before and after deterministic filters."""
...
def figure_candidate_features() -> tuple[Path, Path]:
    """Create Figure 5: component feature distributions for test candidates."""
...
def figure_probability_ranking() -> tuple[Path, Path]:
    """Create Figure 6: top-N precision and recall after probability ranking."""
    metrics = load_metrics()
...
def figure_beam_and_stress() -> tuple[Path, Path]:
    """Create Figure 7: beam-area values and NGC 3031 edge diagnostics."""
    metrics = load_metrics()
    df = pd.concat(
        [
...
def write_captions(files: list[tuple[Path, Path]]) -> Path:
    """Write reusable figure captions with the current numeric results."""
    metrics = load_metrics()
...
These are compact paper-style figures. Each figure is exported as PNG and PDF.
...
**Figure 4. Post-processing metrics.** Patch-level precision and recall before filtering and after connected-component filters. On the test split, beam-area filtering gives precision = {test['precision']:.3f} and recall = {test['recall']:.3f}. On the NGC 3031 stress split, beam-area filtering gives precision = {stress['precision']:.3f} and recall = {stress['recall']:.3f}.

**Figure 5. Candidate component feature distributions.** Morphology and probability features for test-set candidates that survive the final filter. Black curves overlap a catalog label. Red curves are candidate-only components.
...
def main() -> None:
    """Generate all scientific figures and their caption note."""
    setup()
    files = [
        figure_pv_examples(),
