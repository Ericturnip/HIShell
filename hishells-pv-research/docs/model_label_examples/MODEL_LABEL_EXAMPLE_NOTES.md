...
Each shell type has two held-out examples: one with strong pixel-level overlap and one where the patch is detected but the predicted pixels are messy.
...
Use `all_type_examples_grid.png` for a compact overview. Use the individual PNG/PDF files for larger slide panels.
