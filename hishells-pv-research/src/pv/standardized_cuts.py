"""
Build physically standardized position-velocity cuts for U-Net training.
This module turns native THINGS cube samples into fixed 5 kpc by 200 km/s PV
images with the model shape used in the clean physical baseline.
"""
...
from src.utils.wcs_tools import open_cube, pixel_scales_arcsec, velocity_axis_kms


@dataclass(frozen=True)
class StandardCutSpec:
    """
    Describe one requested PV cut before it is sampled from a cube.
    Catalog, offset, velocity-offset, grid, and background cuts use this shared
    format so their metadata can be evaluated by cut category later.
    """
...
def _interp_spectrum_to_axis(
    ...
    *,
    ...
    """
    Resample one line-of-sight spectrum onto the fixed model velocity axis.
    Samples outside the native cube range use a local background value so the
    requested 200 km/s window is padded rather than shortened.
    """
...
def kpc_to_arcsec(window_kpc: float, distance_mpc: float) -> float:
    """Convert a physical width in kpc into an angular width for one galaxy."""
    if distance_mpc <= 0:
...
def spatial_window_pixels(window_kpc: float, distance_mpc: float, pixel_scale_arcsec: float) -> tuple[float, float]:
    """Convert the requested physical PV width into arcsec and image pixels."""
...
def moment1_velocity_map(
    ...
    *,
    ...
    """
    Estimate the local velocity field from the cube itself.
    The cut sampler uses this Moment-1 map to center each standardized velocity
    window on nearby HI emission.
    """
...
def sample_line_coordinates(
    ...
    *,
    ...
    """Return image coordinates for evenly spaced samples along one PV slit."""
...
def local_velocity_for_cut(moment1: np.ndarray, xs: np.ndarray, ys: np.ndarray, *, fallback: float | None = None) -> tuple[float | None, str]:
    """Use the median Moment-1 value along a slit as the velocity center."""
    nx = moment1.shape[1]
    ny = moment1.shape[0]
    vals: list[float] = []
...
def sample_standardized_pv(
    ...
    *,
    ...
    """
    Sample one fixed-shape physical PV cut from a native cube.
    The output keeps the requested model shape even when the slit or velocity
    window reaches an edge of the cube.
    """
...
def deployment_grid_specs(
    ...
    *,
    ...
    """
    Yield blind deployment cuts over the bright part of a moment-0 map.
    This simple image-frame grid is useful for diagnostics; the main training
    script also builds galaxy-frame grids from major and minor axes.
    """
    finite = moment0[np.isfinite(moment0) & (moment0 > 0)]
    if finite.size == 0:
        return
...
def write_standardized_cut(
    ...
    *,
    ...
    """
    Sample and write one standardized PV cut plus its JSON metadata.
    Diagnostic scripts use this when they need a single cut rather than the
    full train, validation, test, and stress dataset.
    """
