...
def _load_names(manifest_path: Path | None, split: str) -> list[str] | None:
    ...
    if not manifest_path.exists():
        print(f"[viz] no manifest found at {manifest_path}; using index-based names.")
        return None
    names = [ln.strip() for ln in manifest_path.read_text().splitlines() if ln.strip()]
    if not names:
        print(f"[viz] manifest {manifest_path} is empty; using index-based names.")
        return None
...
def visualize(
    ...
    """
    Write PV overlays and probability maps for one model split.
    Manifest filenames are used in plot titles when available.
    """
