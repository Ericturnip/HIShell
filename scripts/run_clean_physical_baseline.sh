...
"$TRAIN_PYTHON" -m hishells_pv.data.tensorflow_dataset --config "$CONFIG" --selftest

echo "== 3. Train high-recall clean baseline =="
"$TRAIN_PYTHON" -m hishells_pv.train.tensorflow --config "$CONFIG" --run "$RUN" --quiet --every "$EVERY"
